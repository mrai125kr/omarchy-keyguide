...
install_root=$(mktemp -d "$project_root/qml-bar-widget-install.XXXXXX")
harness=$(mktemp "$project_root/qml-bar-widget-test.XXXXXX.qml")
...
cp tests/qml/bar_widget_harness.qml "$harness"

timeout 10s env \
  KEYGUIDE_TEST_PLUGIN_ROOT="$plugin_root" \
  QT_LOGGING_RULES='qt.qpa.services=false' \
  quickshell --no-color -p "$harness" > build/bar-widget-qml-test.log 2>&1 || {
    cat build/bar-widget-qml-test.log
    exit 1
  }

cat build/bar-widget-qml-test.log
rg -q 'KEYGUIDE_BAR_WIDGET_TEST_PASS' build/bar-widget-qml-test.log
! rg -q 'KEYGUIDE_BAR_WIDGET_TEST_FAIL' build/bar-widget-qml-test.log
